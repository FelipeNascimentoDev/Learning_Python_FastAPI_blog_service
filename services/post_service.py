...
class PostService():
    ...
    async def __get_by_id(self, id: int) -> Record:         # Private method, starts with 'dunder' --> '__'
    ...
    async def update(
            self,
            id: int,
            post: PostUpdateRequest) -> Record:
        total = await self.count(id)    # The count() method checks if the post exists (return 1 OR 0)
        if not total:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Post not found")
        
        data = post.model_dump(exclude_unset=True)  # Take 'PostUpdateRequest' and remove what is 'None' to only save altered data
